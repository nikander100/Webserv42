...
Server::Server() : _servername(""), _port(0), _host(0), _root(""),
	_clientMaxBodySize(MAX_CONTENT_SIZE), _index(""), _listenFd(0), _autoindex(false) {
	initErrorPages();
}

Server::Server(const Server& copy) : _servername(copy._servername),
	_port(copy._port), _host(copy._host), _root(copy._root),
	_clientMaxBodySize(copy._clientMaxBodySize), _index(copy._index), _listenFd(copy._listenFd),
